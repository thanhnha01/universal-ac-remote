...
- `SupportedProtocols.md` is stored under `data/upstreams/snapshots/irremoteesp8266/` at the same full commit. [generate_protocol_catalog.py](../tools/generate_protocol_catalog.py) emits [PROTOCOL_CATALOG.md](PROTOCOL_CATALOG.md): generic-enabled families, detailed families needing sender/capability validation, and detailed variants outside `IRac::isProtocolSupported()`. Catalog discovery never automatically admits a new protocol to the APK.
